# **UPDATE 26-07-26**

**🟢 New :**
- Lirik Yukoma Thermae (onsen)
- Impor nama skill dari global
- Deskripsi skill dengan detail kondisi
- Menu Bantuan
**Aset Grafis :**
- Animasi teks hasil karir, reroll spark, dan transfer request
- Impor aset nama balapan dari global

**🟠 Improved:**
- Impor data dari global : nama skill, gelar, julukan
- Edit config
- Menu pengaturan
- Koreksi
- Perbaikan aset grafis lainnya

# **UPDATE 15-07-26**

## Skenario
**🟢 New :**
- Teks sistem Ramen Tracen (aset grafis menyusul)
- Teks sistem Aoharu Hai/Unity Cup
- Teks sistem Make a New Track/Trackblazer

## Lain-lain
- Menu log skill
- Menu skill Evo
- Menu reroll spark
- Koreksi & sorting code
- Impor data dari global : Nama skill, Gelar, Julukan


# **UPDATE 02-07-26**

## Lirik
**🟢 New :**
- Tracen Ramen Retsuden
- Gaze on Me
- Glorious Moment
- Namida Hikatte Asu ni Nare
- U.M.A New World
- Pyoitto Hallelujah
- Beyond the Future
- winning the soul
- STARTING FORCE
- Girls Legend U
- Soshite Minna No

**🟠 Improved:**
- Make Debut
- O-Rorize
- Tucker Grindin' Island
- Umapyoi Densetsu

## Cerita/story
🟢 **NEW:**
- Buena Vista **(Clear)**
- Verxina **(Clear)**
- Agnes Digital **(Clear)**
- Cesario **(Clear)**
- Curren Bouquetd'or (Episode 1-4)
- Admire Groove (Episode 1-4)
- Prolog skenario Dreams

🟠 **Improved:**
**Main Story**
- Mejiro McQueen Arc (Fix with Cutscene)
- Part 2 Chapter 1 (Fix Episode 1-5)

## Dialog
**Beranda:**
- Nice Nature
- Ikuno Dictus
- Buena Vista
- Kitasan Black
- Daring Heart
- Espoir City
**Dialog Balapan berturut-turut**

## Aset Grafis
🟢 **NEW:**
- Skenario Pulau
- Skenario Onsen
- Skenario Dreams
- Komik (50%)
- Race G1
- Menu Kustom/custom race

🟠 **Improved:**
-  Beranda
- Menu Gacha
- Training/Karir
- Team Trials

## Repositori
- Nambahin info repo + ikon di hachimi
- Edit config > persiapan migrasi
- Edit readme > deskripsi repo
- Update Wiki & Status TL (updated 22/05)

## Perbaikan 🛠️
- Bug error 205 di agenda/jadwal
- Bug atlas error (bundle diperbarui)
- Beberapa teks nabrak/hilang
- Koreksi miss TL & typo sekaligus rapihin UI
